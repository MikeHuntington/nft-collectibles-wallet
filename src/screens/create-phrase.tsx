import React, {FC, useEffect, useState} from 'react'
import {useSelector, useDispatch} from 'react-redux'
import {StackNavigationProp} from '@react-navigation/stack'
import styled from 'styled-components/native'

import { RootState } from '../redux/reducers'

import { AuthStackParamList, AppScreens} from '../navigation/AuthNavigator'

import { CheckBox } from 'react-native-elements'
import Text from '../components/Text'
import Button from '../components/Button'
import { generatePhrase } from '../redux/actions'

type CreatePasswordNavigationProps = StackNavigationProp<AuthStackParamList, AppScreens.CreatePassword>;

interface CreatePasswordProps {
    navigation: CreatePasswordNavigationProps;
}

const CreatePhrase : FC<CreatePasswordProps> = ({navigation}) => {
    const dispatch = useDispatch()
    const [phraseSaved, setPhraseSaved] = useState(false)
    const { phrase } = useSelector((state: RootState) => state.keyring)

    useEffect(() => {
        dispatch(generatePhrase())
    }, [])

    return (
        <Container>
            <Main>
                <ScreenTitle title bold>Create Wallet</ScreenTitle>
            </Main>

            <Divider />

            <MnemonicSeed>
                <Text color={"silver"} title center heavy>{phrase?.phrase}</Text>
            </MnemonicSeed>

            <Warning>
                <Text heavy small center color="brown">
                    Never share this phrase with anyone. Make sure you write it down and store it in a safe place.
                    If you lose this code, you will lose access to your items.
                </Text>
            </Warning>

            <Divider />

            <CheckBoxContainer>
                <CheckBox
                    center
                    title='I have written down my private phrase and saved it somewhere safe.'
                    containerStyle={{ backgroundColor: 'transparent', borderColor: 'transparent' }}
                    checked={phraseSaved}
                    onPress={() => setPhraseSaved(!phraseSaved)}
                    />
            </CheckBoxContainer>
            
            <Spacer />

            <Button disabled={!phraseSaved}>
                Continue
            </Button>

            <StatusBar barStyle="light-content" />
        </Container>
    )
}

export default CreatePhrase;

/* Styles */
const Container = styled.View`
    flex: 1;
    justify-content: center;
    align-content: center;
    /*background-color: rgb(41, 18, 71);*/
`;

const Main = styled.View`
    margin: 22px 32px 0px;
`;

const ScreenTitle = styled(Text)``


const StatusBar = styled.StatusBar`

`;

const Warning = styled.View`
    margin: 34px 32px 0 32px;
    padding: 30px;
    background-color: #ebd7ad;
    border-radius: 10px;
`

const CheckBoxContainer = styled.View`
    margin: 0 32px 0 32px;
`

const CreateWalletButton = styled.TouchableOpacity`
    margin-top: 16px;
`

const MnemonicSeed = styled.View`
    margin: 20px 0 20px 0;
`

const Spacer = styled.View`
    flex: .4;
`

const Divider = styled.View`
    border-bottom-color: #dddddd;
    border-bottom-width: 2px;
    margin: 30px 32px 32px;
`